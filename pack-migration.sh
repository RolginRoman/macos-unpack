...
if [ -z "$ARCHIVE" ]; then
  echo "Usage: $0 unpack-migration.sh <archive.tar.gz>"
  exit 1
fi
...
echo "[3/14] Packing Homebrew packages list..."
brew bundle dump --file="$BACKUP_DIR/Brewfile" --force 2>/dev/null || echo "Homebrew not found, skipping"

echo "[4/14] Packing NVM version info..."
nvm version > "$BACKUP_DIR/nvm-version.txt" 2>/dev/null || echo "24" > "$BACKUP_DIR/nvm-version.txt"
npm list -g --depth=0 > "$BACKUP_DIR/npm-global-packages.txt" 2>/dev/null || true

echo "[5/14] Packing Rust toolchain info..."
...
echo "[12/14] Packing database dumps..."
...
echo "[13/14] Packing macOS defaults..."
...
echo "[14/14] Saving recommended packages list..."
